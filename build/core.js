	...
	return {
		...
		preload: function (name) {
			...
			return function () {
				...
				Core.evt.progress.map(function (p) {
				...
				if (assetsToLoad === 0 && pageLoaded) {
					...
					preloading = false;
					Core.evt.onloads.map(function (o) {
						o();
					});
				}
...
(function(Core){
  console.info('Simple JavaScript Inheritance');
...
(function (Core) {
	...
	Core.timer = function (time, cb, done) {
...
(function (Core) {
	...
	colors = {
		...
		get: function (idx) {
			return palette[Core.utils.clamp(idx % palette.length, 0, palette.length - 1)];
		},
...
(function (Core) {
	...
	input = {
		...
		init: function (dom, icade) {
			...
			icade = icade || Core.urlParams.icade;
...
(function (Core) {
	...
	var Image = Core.Class.extend({
		...
		init: function (path, flipFlags, scale) {
			...
			Core.gfx.loadImage(path, function (img){
...
(function (Core) {
    ...
    var Tiled = Core.Class.extend({
        ...
        init: function (file, onload) {
            ...
            Core.utils.ajax(file, function (xhr) {
        ...
        layerByName: function (name) {
            ...
            var layer = Core.utils.getByKeyValue(this.layers, "name", name);
            return layer ? [layer] : [];
...
(function (Core) {
	...
	var TrackingCamera = Core.Camera.extend({
		...
		tick: function () {
			...
			var center = Core.utils.center(this, this.zoom),
				e = this.entity,
				xr = this.xRange,
				yr = this.yRange;
		...
		render: function (gfx, renderables) {
			...
			this._super(gfx, renderables.concat([{
				render: function (gfx, cam) {
					...
					var center = Core.utils.center(cam, cam.zoom);
	...
	Core.TrackingCamera = TrackingCamera;
...
(function (Core) {
	...
	var Dialog = Core.Class.extend({
		...
		tick: function (delta) {
			...
			if (this.killKey && Core.input.pressed(this.killKey)) {
				Core.input.release(this.killKey);
				this.done();
			}
...
(function (Core) {
	...
	*/
	var Velocity = Core.Trait.extend({
	...
	var Gravity = Core.Trait.extend({
		...
		tick: function (t) {
			...
			if (this.falling) {
				t.accY += 0.25;
				t.accY = Core.utils.clamp(t.accY, 0, 20);
			} else {
				t.accY = 0;
			}
	...
	*/
	var RemoveAfter = Core.Trait.extend({
	...
	*/
	var Sin = Core.Trait.extend({
		...
		tick: function (t) {
			...
			this[t.target] += Math.sin(Core.utils.now() / t.speed) * (t.amp / 10);
...
(function (Core) {
	...
	var SpriteSheet = Core.Class.extend({
...
(function (Core) {
    ...
    var SpriteAtlas = Core.Class.extend({
        ...
        initPList: function (plist) {
            ...
            var resolve = Core.preload(plist);
            Core.utils.ajax(plist, function (xhr) {
        ...
        initCSV: function (path) {
            ...
            var resolve = Core.preload(path);
            Core.utils.ajax(path + ".txt", function (xhr) {
                Core.gfx.loadImage(path + ".png", function (img) {
        ...
        loadImages: function (imageData) {
            ...
            imageData.forEach(function (imgData) {
                Core.gfx.loadImage(self.path + imgData.path, function (img) {
...
(function (Core) {
	...
	var Anim = Core.Class.extend({
		...
		tick: function () {
			...
			var diff = Core.utils.now() - this.frameTime;
			this.changed = false;
			this.rewound = false;

			if (diff > this.speed) {
				this.frameTime = Core.utils.now() + (Math.min(this.speed, diff - this.speed));
...
(function (Core) {
	...
	var Map = Core.Class.extend({
		...
		getBlockEdge: function(pos, vertical) {
		    ...
		    return Core.utils.snap(pos, snapTo);
		...
		*/
		imgToCells: function (img, colourMap, cb, flipFlags) {
			...
			if (typeof img === "string") {
				// Load first
				Core.gfx.loadImage(img, function (canvas){
...
(function (Core) {
	...
	var DebugMap = Core.Map.extend({

		init: function (tileW, tileH, xTiles, yTiles, cells, walkable, seed) {

			var ctx = this.ctx = Core.gfx.createCanvas(tileW * xTiles, tileH * yTiles),
				data = Core.gfx.ctx.createImageData(tileW * xTiles,tileH * yTiles),
				pix = data.data,
			...
			oldSeed = Core.utils.rnd.seed;
			Core.utils.rnd.seed = this.seed;
			off = Core.utils.rnd.rand(255);

			for (var i = 0; i < numPix; i++) {
				var row = i / data.width | 0,
					col = ((i / tileW) | 0) % data.width % xTiles,
					noise = Core.utils.rnd.rand(100) < 30 ? (Core.utils.rnd.rand(30)) : 0,
			...
			Core.utils.rnd.seed = oldSeed;

			ctx.putImageData(data, 0, 0);

			this._super(
				new Core.SpriteSheet(ctx.canvas, tileW, tileH),
				cells,
				walkable);
...
(function (Core) {
	...
	RayCastMap = Core.Map.extend({
		...
		setStripWidth: function (stripWidth) {
			...
			this.numRays = Math.ceil(Core.env.w / stripWidth);
		...
		setFOV: function (fov) {
			...
			var fovRad = Core.utils.deg2rad(fov);
			this.viewDistance = (Core.env.w / 2) / Math.tan((fovRad / 2));
		...
		castRays: function (entities, p) {
			...
			for (i = 0; i < this.numRays; i++) {
				...
				hit = Core.rays.cast(p.rotation + rayAngle, p.x + p.w / 2, p.y + p.h / 2, this, entities);
...
(function (Core) {
	...
	var Game = Core.Class.extend({
		...
		init: function (w, h) {
			...
			Core.env.w = ctx.canvas.width;
			Core.env.h = ctx.canvas.height;

			Core.gfx.init(ctx);
			Core.input.init(ctx.canvas);

			Core.evt.onload(function () {
				self.load();
				self.run(Date.now());
			});
		...
		run: function () {
			...
			var frate = $('#frame_rate');
			if(frate.val() == undefined || frate.val() <= 1)
				this.accumulator += frameTime;
			else{
				this.accumulator = frameTime * frate.val();
			}
			if (this.running) {
				c = 0;
		...
		setScreen: function (screen, opts) {
			...
			if (this.screenPrev) {
				...
				Core.timer(opts.time || 20, function (ratio) {
